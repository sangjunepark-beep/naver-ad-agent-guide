...
**2026-04-20 재발**: Opus로 붙은 채 sync_memory·n8n 노드 버그 분석·워크플로우 PUT 패치·대표 보고서 HTML 작성·로드맵 정리 등 Sonnet 범위 작업을 세션 내내 진행. 사용자가 "이거 sonnet하고 opus 같이 쓰는건가"라고 직접 물을 때까지 선제 고지 없음. 2회 연속 규칙 위반.

**How to apply:** 첫 응답 전 반드시 이 체크를 돌릴 것. 특히 Opus로 붙어 있으면 "정말 Opus 필요한가?"를 한 번 더 자문. **n8n 디버깅·문서 작성·메모리 관리는 전부 Sonnet 범위임을 암기.** Opus는 Phase 설계·리스크 레지스터·change-request 수준에서만.
